...
?>
    ...
    <div class="topbar">
        <div>
            <div class="breadcrumb"><span>/</span> pay history</div>
            <h2>Pay History</h2>
        </div>
    </div>
    ...
    <div class="content">
        <div class="table-wrap">
            ...
            <table>
                <thead>
                    <tr>
                        <th class="sortable" data-col="BusinessEntityID">Employee ID<span class="sort-icon"></span></th>
                        <th class="sortable" data-col="FullName">Name<span class="sort-icon"></span></th>
                        <th class="sortable active-desc" data-col="RateChangeDate">Effective Date<span class="sort-icon"></span></th>
                        <th class="sortable" data-col="Rate">Pay Rate<span class="sort-icon"></span></th>
                        <th>Frequency</th>
                    </tr>
                </thead>
                <tbody id="pay-tbody">
    ...
    <script>
        let currentPage   = 1;
        let currentSearch = '';
        let currentSort   = 'RateChangeDate';
        let currentDir    = 'DESC';
        let debounceTimer = null;

        function fetchPayHistory(page, search, sort, dir) {
            currentPage   = page;
            ...
            document.getElementById('pay-tbody').style.opacity = '0.4';

            fetch('pay_history_data.php?' + params.toString(), {
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
                .then(res => res.json())
                .then(data => {
                    document.getElementById('pay-tbody').style.opacity = '1';
                    renderTable(data);
                    renderPagination(data);
                    updateSortHeaders(data.sort, data.dir);
                    document.getElementById('row-count').textContent =
                        Number(data.totalRows).toLocaleString() + ' pay records';
                ...
                .catch(() => {
                    ...
                    document.getElementById('pay-tbody').innerHTML =
                        '<tr><td colspan="5" class="empty">Failed to load data. Check your connection.</td></tr>';
                });
        }
        ...
        function renderTable(data) {
            ...
            if (!data.rows || !data.rows.length) {
                tbody.innerHTML = '<tr><td colspan="5" class="empty">No pay records found.</td></tr>';
                return;
            }
            ...
            tbody.innerHTML = data.rows.map(r => `
                <tr>
                    ...
                    <td class="mono">${esc(r.RateChangeDate)}</td>
                    <td class="mono" style="color:var(--accent);">$${esc(r.Rate)}</td>
                    <td>
        ...
        function btn(page, label, active = false) {
            return `<button class="page-btn ${active ? 'active' : ''}" onclick="fetchPayHistory(${page}, currentSearch, currentSort, currentDir)">${label}</button>`;
        }
        ...
        document.querySelectorAll('th.sortable').forEach(th => {
            th.addEventListener('click', () => {
                ...
                fetchPayHistory(1, currentSearch, col, dir);
            });
        });
        ...
        document.getElementById('search-input').addEventListener('input', function () {
            ...
            debounceTimer = setTimeout(() => {
                fetchPayHistory(1, this.value.trim(), currentSort, currentDir);
            }, 350);
        });

        function clearSearch() {
            document.getElementById('search-input').value = '';
            fetchPayHistory(1, '', currentSort, currentDir);
        }
