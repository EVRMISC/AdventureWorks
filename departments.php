...
?>
    ...
    <div class="topbar">
        <div>
            <div class="breadcrumb"><span>/</span> departments</div>
            <h2>Departments</h2>
        </div>
    </div>
    ...
    <div class="content">
        <div class="table-wrap">
            <div class="table-header">
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                    <div class="search-box">
                        ...
                        <input type="text" id="search-input" placeholder="Search name or group...">
                    </div>
            ...
            <table>
                <thead>
                    <tr>
                        <th class="sortable" data-col="DepartmentID">ID<span class="sort-icon"></span></th>
                        <th class="sortable active-asc" data-col="Name">Department Name<span class="sort-icon"></span></th>
                        <th class="sortable" data-col="GroupName">Group<span class="sort-icon"></span></th>
                        <th class="sortable" data-col="HeadCount">Headcount<span class="sort-icon"></span></th>
                    </tr>
                </thead>
                <tbody id="dept-tbody">
    ...
    <script>
        ...
        function fetchDepartments(page, search, sort, dir) {
            currentPage   = page;
            ...
            document.getElementById('dept-tbody').style.opacity = '0.4';

            fetch('departments_data.php?' + params.toString(), {
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
                .then(res => res.json())
                .then(data => {
                    document.getElementById('dept-tbody').style.opacity = '1';
                    renderTable(data);
                    renderPagination(data);
                    updateSortHeaders(data.sort, data.dir);
                    document.getElementById('row-count').textContent =
                        Number(data.totalRows).toLocaleString() + ' departments';
                ...
                .catch(() => {
                    ...
                    document.getElementById('dept-tbody').innerHTML =
                        '<tr><td colspan="4" class="empty">Failed to load data. Check your connection.</td></tr>';
                });
        }
        ...
        function renderTable(data) {
            ...
            tbody.innerHTML = data.rows.map(r => `
                <tr>
                    ...
                    <td>${esc(r.Name)}</td>
                    <td style="color:var(--text-muted);font-size:13px;">${esc(r.GroupName)}</td>
                    <td>
                        <span class="badge ${parseInt(r.HeadCount) > 0 ? 'badge-green' : 'badge-gray'}">
        ...
        function btn(page, label, active = false) {
            return `<button class="page-btn ${active ? 'active' : ''}" onclick="fetchDepartments(${page}, currentSearch, currentSort, currentDir)">${label}</button>`;
        }
        ...
        document.querySelectorAll('th.sortable').forEach(th => {
            th.addEventListener('click', () => {
                ...
                fetchDepartments(1, currentSearch, col, dir);
            });
        });
        ...
        document.getElementById('search-input').addEventListener('input', function () {
            ...
            debounceTimer = setTimeout(() => {
                fetchDepartments(1, this.value.trim(), currentSort, currentDir);
            }, 350);
        });

        function clearSearch() {
            document.getElementById('search-input').value = '';
            fetchDepartments(1, '', currentSort, currentDir);
        }
